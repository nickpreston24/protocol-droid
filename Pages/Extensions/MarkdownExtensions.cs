...
{
    public static string[] MarkdownToHtml(this string[] lines)
    {
        // https://regex101.com/r/gVbc8C/1
        string markdown_links_pattern = @"\[(?<text>.*)\]\((?<url>.*(""(?<alt>.*)"")?)\)";
...
public class MarkdownRegexPatterns
{
    ...
    public readonly static Dictionary<int, string> FrontMatter = new string[]
        {
            """(?<label>^[a-zA-Z_\s]+):(?<value>.*\s*?)""", """(?<label>\w+:)(?<value>.*)"""
        }
        .Select((s, index) => new { s, index })
        .ToDictionary(x => x.index, x => x.s.Trim());
